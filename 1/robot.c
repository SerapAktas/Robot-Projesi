...
#define sensor5  GPIO_PIN_5
#define sensor6  GPIO_PIN_1
#define sensor7  GPIO_PIN_2
#define sensor8  GPIO_PIN_3
unsigned long ulPeriod, sag_motor, sol_motor,sagMotorHizi=600,solMotorHizi=130;
...
{
	if( (!GPIOPinRead(GPIO_PORTA_BASE,sensor1)))
			{
			 ...
			 else  if((!GPIOPinRead(GPIO_PORTE_BASE,sensor2)))
			{
			 ...
			 else if( (!GPIOPinRead(GPIO_PORTA_BASE,sensor3)))
			{
	...
	if (sagMotorHizi < 80) sagMotorHizi = 80; 
  if (solMotorHizi < 40 ) solMotorHizi = 40;
	if (sagMotorHizi >= 600) sagMotorHizi = 600; 
  if (solMotorHizi >= 190) solMotorHizi = 190; 
...
ADCIntClear(ADC0_BASE, 1);
	...
	GPIOPinWrite(GPIO_PORTE_BASE, GPIO_PIN_0,0 ); //ir ledler sonük
	GPIOPinWrite(GPIO_PORTD_BASE, GPIO_PIN_3,0 ); // ir ledler sönük
	TimerDisable(TIMER2_BASE, TIMER_BOTH); 
	 GPIOPinTypeGPIOInput(GPIO_PORTA_BASE,sensor1|sensor3|sensor4);
	  GPIOPinTypeGPIOInput(GPIO_PORTE_BASE,sensor2|sensor5|sensor6|sensor7|sensor8);		//led
		
		GPIOPadConfigSet(GPIO_PORTA_BASE,sensor1|sensor3|sensor4,GPIO_STRENGTH_2MA,GPIO_PIN_TYPE_STD_WPU);
		GPIOPadConfigSet(GPIO_PORTE_BASE,sensor2|sensor5|sensor6|sensor7|sensor8, GPIO_STRENGTH_2MA,GPIO_PIN_TYPE_STD_WPU);
...
mot_oran1=500;
	...
	f22=900;
		...
		if(run==1)
		{
			if(run1==1)
			{							
				if(f1>70)					//sagda engel varsa
				{
					if(uzak<80)					//onde engel varsa
					{
						if(f2>70)					//solda engel varsa
						{		oku_say=0;
									while( uzak <80)
										duz_ileri();	
						    	oku_say=0;				//çikmaz sokak
									sol();
									while(oku_say<45)
									{							
									}
